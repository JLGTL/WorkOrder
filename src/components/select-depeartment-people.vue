<template>
    <div class="departCont">
        <Head :title="title" @back="back" :hasBorder="true" :sure="true" @sureReturn='sureReturn' v-if="isChecked!=-1"/>
        <Head :title="title" @back="back" :hasBorder="true" v-if="isChecked==-1"/>
        <!-- <van-search placeholder="请输入部门名称" v-model="searchValue" @focus="onFocus" class="search" v-if="searchType=='dep'">
			<template slot="left-icon">
			    <img src="../assets/images/search_icon.png" class="icon"/>
			</template>
		</van-search> -->
        <!-- <p class="van-cell flex select-title" v-if='searchType=="per"'>
            <span class="depart-title">部门目录</span>
            ...
            <div class="depart-list" v-if='searchType=="per"'>
                <div class="list-item">
                    <p class="van-cell flex depart-one" v-for="(depart,index) in personList" @click="selectPer(depart.id,index)">
                        <span class="depart-user-name">
                            <img src="../assets/images/head.png" alt="" class="headImg" v-if="!depart.img">
                            <img :src="depart.img" alt="" class="headImg" v-else>
                            {{depart.name}}
                        </span>
                        <span class="depart-user-title">工单接收人</span>
						<span class="checkTip" :class="[isChecked==index?'isCheckedTip':'']"></span>
                    </p>
                </div>
    ...
    export default {
        ...
        methods: {
			...
			sureReturn(){
				...
				this.$emit('selectExecute',execute);
				this.back();
			},
            back(){ 
				// this.departList = [];
