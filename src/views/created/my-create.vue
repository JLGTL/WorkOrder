                ...
                <template slot="1">
                    <div class="tab-list">
                        <div class="list" :class="{'haveLoaded':haveLoaded,'noMore':noMore}">
                            <mt-loadmore :top-method="loadTop" :bottom-method="loadBottom" :bottomAllLoaded="allLoaded" :auto-fill="false" ref="loadmore" bottomPullText="上拉加载" bottomDropText="释放加载">
                                <div class="tab-list-item p-lr" v-for="(item,index) in inProcessData" v-if="inProcessData.length>0">
                                    <div v-if="tabActiveIndex == 2" @click="enterDetail(2,item.id)">
                                        ...
                                        <div class="list-item-title flex"><p>{{item.typename}}&ensp;|&ensp;<span class="color-gray">{{item.title}}</span></p>
                                            <p class="sign_normal sign_status" v-if="item.status == 2">待确认</p>
                                            <p class="sign_important sign_status" v-if="item.status == 3">处理中</p>
                                            <p class="sign_delay sign_status" v-if="item.status == 9">已延误</p>
                                        </div>
                                        ...
                                        <p class="list-item-create flex">
                                            ...
                                            <span class="time" v-if="item.status == 3 || item.status == 9"><i v-if="item.sj">计时 </i>
                                                <i class="sign_important" v-if="item.status == 3">{{item.sj}}</i>
                                                <i class="sign_outtime" v-if="item.status == 9">{{item.sj}}</i>
                                            </span>
                                        </p>
                                    </div>
                                    <div v-else-if="item.isReturn == 0 && tabActiveIndex == 1" @click="enterDetail(1,item.id)"><!--抄送给我但工单执行人有不是退回状态-->
                                    ...
                                    <div v-else-if="item.isReturn == 1 && tabActiveIndex == 1" @click="enterDetail(1,item.id)" style="position: relative"><!--抄送给我的退回的状态码-->
                                        <van-swipe-cell :right-width="50" :left-width="0" :on-close="onClose">
                                            <van-cell-group>
                                                <div class="tag_normal" v-if="item.importantType == '普通'">普通</div>
                                                <div class="tag_important" v-if="item.importantType == '重要'">重要</div>
                                    ...
                                    <div v-else-if="tabActiveIndex == 3" @click="enterDetail(4,item.id)">
                                        <div class="tag_normal" v-if="item.importantType == '普通'">普通</div>
                                        <div class="tag_important" v-if="item.importantType == '重要'">重要</div>
                                        <div class="tag_danger" v-if="item.importantType == '紧急'">紧急</div>
                                        <div class="list-item-title flex"><p>{{item.typename}}&ensp;|&ensp;<span class="color-gray">{{item.title}}</span></p>
    ...
    export default {
		...
		beforeRouteEnter(to, from, next){
			...
			next(vm => {
				...
				if(from.name!='CreateDetails' && from.name!='CreateTobesend'){
					vm.active = '';
					vm.defaultTab = 1;
					vm.showTabIndex = 1;
					vm.filterShow = false;
					...
					clearInterval(vm.timer);
					vm.timer = null;
					console.log('vm.tabActiveIndex',vm.tabActiveIndex)
					vm.initTabList();
					vm.searchCondition = {
						pageNo:1,
						pageSize:10
        ...
        methods: {
            ...
            initList(type){
                ...
                this.axios({
                    ...
                    console.log(self.inProcessData);
                    ...
                    if(self.inProcessData.length>0){
                        self.inProcessData.forEach((item)=>{
                            if(type == 2 && item.status == 3){
                                self.$set(
                                    item,"sj",self.getRemainderTime(item.confirmTime)
                                );
                            }
                            if(type == 3 && item.status == 4){
                                self.$set(
                                    item,"sj",self.getRemainderTime(item.beginTime,2)
                                );
                            }
                            if(item.status == 9){
                                self.$set(
            ...
            enterDetail(index,id){
                if(index == 1){
                   var path = '/createtobesend'
                }else{
                   var path = '/createdetails'
                }
                this.$router.push({
                    query:{
                        orderId:id,
