<template>
    <div class="backGrey mainpage">
        <Head title='选择人员' :sure="true" @sureReturn='sureReturn' :hasBorder="true"  @back="back"/>
        <div class="backGrey mat54 searchBox">
            <form action="javascript:return true">
                <van-search placeholder="请输入姓名/工号/拼音" v-model="value" :clearable='true' @search="searchPerson" :show-action='true' @focus="openStaffsBack" @cancel="closeStaffBack" id="myBox" class="search">
        ...
        <div class="group backWhite">
            ...
            <div class="showStaffList flex" v-if="showStaffList.length>0">
                <!--<div class="showStaff" v-if="copyPeopleColums[0]">-->
                    <!--<img class="flexIn avatar" :src="copyPeopleColums[0].avatar" v-if="copyPeopleColums[0].avatar"/>-->
                    <!--<img class="flexIn avatar" src="../../assets/images/head.png" v-if="!copyPeopleColums[0].avatar"/>-->
                    <!--<span class="flexIn">{{copyPeopleColums[0].name}}</span>-->
                <!--</div>-->
            ...
            <div class="title flex" v-if="nodes.length==0&&staffs.length==0&&scrollList.length==0">
                <span class="flexIn icon-group"></span>
                <span class="groupname">华图组织架构</span>
                <i class="line"></i>
            </div>
            <ul class="glist glist1" v-if="nodes.length==0&&staffs.length==0&&scrollList.length==0">
                <li class="flex" @click="loadMore(depId,depName)" >
                    <img class="flexIn avatar avatar2" src="../../assets/images/organizition@2x.png" style="border-radius: 0" />
                    <span class="flexIn name">{{depName}}</span>
            ...
            <div class="allscroll2" v-if="scrollList.length>0" :class="{'noSelectPeople':nostyle}">
...
<script>
    ...
    // import {organizeStructure,fuzzySearchLists} from '@/api/architecture';//架构一级列表
    import { Toast,Dialog } from 'vant';
    export default {
        // props:['selectlist','copyPeopleColums'],//抄送人id，抄送人
        name: 'SendDepeartmentPeople',
        components: {
            Head
        },
        data() {
        ...
        created() {
            ...
            //this.selectlist = localStorage.getItem('selectlist');
            console.log("000===000===")
//            this.copyPeopleColums.forEach(item=>{
//                let staInList ={};
//                staInList.name = item.name;
//                staInList.id = item.copyperson;
//                staInList.avatar = item.avatar;
//                staInList.type = 0;
//                this.showStaffList.push(staInList)
//            })
//            this.showStaffList.shift()
            this.initDepartment();
        },
        watch:{
//            value:function () {
...
//            },
            ...
            showStaffList:function(){
                ...
                if(this.showStaffList.length == 0){
                    this.nostyle = true
                }else{
                    this.nostyle = false
                }
        ...
        computed: {
            selectlist2() {
                ...
                if(this.selectlist.indexOf(",")>-1){
                    select = this.selectlist.split(",");
                }else if(this.selectlist){
                    select.push(this.selectlist);
                }
                return select;
            }
        },
        ...
        methods:{
            ...
            initDepartment(){
                ...
                self.axios({
                    url:'/api/v1.0/selecttransmitdepartment',
                    method:'post',
                    data:data,
                }).then(function(res){
                    console.log(res);
                    self.depId = res.data.data.id;
                    self.depName = res.data.data.name;
                })
            },
            // 滑动关闭键盘
            ...
            onLoad(){
                ...
                let data = {
                    ...
                    pageNum:self.page,
                    pageSize:20,
                    departmentId:self.depId
                }
                if(this.value){
                    console.log(self.value);
                    self.axios({
                        url:'/api/v1.0/find_ehr_by',
                        method:'post',
                        data:data
                    }).then(res => {
                        console.log(res)
            ...
            sureReturn(){
                ...
                let self =this;
                let data =[];
//                data.push(this.copyPeopleColums[0])
                this.showStaffList.forEach(item=>{
                    let staInList ={};
                    staInList.name = item.name;
                    staInList.copyperson = item.id;
                    staInList.avatar = item.avatar;
                    staInList.type = 0;
                    data.push(staInList)
                })
...
//                localStorage.setItem('copyPeopleColums',JSON.stringify(data))
//                this.$router.push({
//                    path:'/myexecute'
//                    path:'/orderdetails',
//                    query:{
//                        orderId : self.orderId,
//                        showStaffId: self.showStaffList[0].id,
//                        showStaffName: self.showStaffList[0].name,
//                    }
//                })
                // this.$emit('confirmcopypeople',data)
                // this.back();

                if(self.showStaffList.length == 0){
                    Toast('请先选择执行人')
                    return false;
                }
                let postData = {
                    id:self.orderId,
                    userId:self.userId,
                    toId: self.showStaffList[0].id,
                    toName:self.showStaffList[0].name,
                }
...
//                this.showStaffList.forEach(item=>{
//                    if(parseInt(this.list[index].id)==parseInt(item.id)){
//                        hasThisPerson = true;
//                    }
//                })
                this.showStaffList = [];
                if(!hasThisPerson){
                    ...
                    staInList.id = this.list[index].id;
                    staInList.avatar = this.list[index].avatar;
                    staInList.type = 0;
                    this.showStaffList.push(staInList);

                    this.staffs.forEach(item=>{
                        if(parseInt(item.id)==parseInt(staInList.id)){
                            item.isChecked = true;
                        }
                    })
                }
...
//                if(!this.staffs[index].canChecked){
//                    Toast("直属领导为必须抄送人");
//                    return false;
//                }
                //选中
                ...
                this.staffs[index].isChecked = true;
                let item = {
...
//                        if(staff.id==this.staffs[index].id){
...
//                }
            ...
            removeStaff(index){
                ...
                this.staffs.forEach(sta=>{
                    if(sta.id==this.showStaffList[index].id){
                        ...
                        if (res.data.code == 200) {
                            ...
                            if(res.data.data.users){
                                res.data.data.users.forEach((item,index)=>{
                                    console.log("self.selectlist2")
                                    console.log(self.selectlist2)
                                    if(self.selectlist2){
                                        let isselect = false;
                                        self.selectlist2.forEach((sel,index2)=>{
                                            if(parseInt(item.id)==parseInt(sel)){
                                                isselect = true;
                                            }
                                        })
                                    ...
                                    //选中直属领导
//                                    if(self.copyPeopleColums){
//                                        if(parseInt(self.copyPeopleColums[0].copyperson) == parseInt(item.id)){
//                                            item.isChecked = true;
//                                            item.canChecked = false;
//                                        }
//                                    }
                                    //选中列表里有的 selectlist2里没有的
                                    self.showStaffList.forEach(sta=>{
                                        if(parseInt(item.id)==parseInt(sta.id)){
                                            item.isChecked = true;
                                        }
                                    })
