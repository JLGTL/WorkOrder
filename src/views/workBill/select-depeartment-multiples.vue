	...
	<div class="backGrey mainpage">
		...
		<div class="group backWhite">
			...
			<ul class="glist glist1" v-if="nodes.length==0&&staffs.length==0&&scrollList.length==0">
				<li class="flex" @click="loadMore(1,'北京华图宏阳教育文化发展股份有限公司')" >
					<img class="flexIn avatar avatar2" src="../../assets/images/organizition@2x.png" style="border-radius: 0" />
					<span class="flexIn name">北京华图宏阳教育文化发展股份有限公司</span>
			...
			<div class="allscroll2" v-if="scrollList.length>0">
				...
				<ul class="glist glist2" v-if="nodes.length>0">
					<li :class="[index+1==nodes.length&&staffs.length==0?'flex noborder':'flex']" v-for="(item,index) in nodes" @click="loadMore(item.id,item.name)" >
...
.selectperson{
	width:12px;
	height:12px;
	background: url(../../assets/images/uncheck@2x.png) 0 0 no-repeat;
...
.selectperson.selected{
	...
	/* width: 10px;
	height: 10px; */
	background: url(../../assets/images/checked@2x.png) 0 0 no-repeat;
	background-size: 100% 100%;
}
.staffsBack{
	position: fixed;
